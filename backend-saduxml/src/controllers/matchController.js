...
import { Match, Group, Team, Member, Stage } from "../db.js";
...
export const createMatch = async (req, res) => {
  try {
    ...
    await Match.create({ group_id, team1_id, team2_id, winner_id, match_date, status, score_team1, score_team2, stage_id });
...
export const listMatches = async (req, res) => {
  ...
  try {
    const matchList = await Match.findAll({
      ...
      where: {
        [Op.or]: [
          // bisa cari berdasarkan group name, atau nama tim
          { '$Group.name$': { [Op.like]: `%${keyword}%` } },
