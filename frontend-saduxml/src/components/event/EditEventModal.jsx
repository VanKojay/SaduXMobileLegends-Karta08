...
import { X, Calendar, Users, MapPin, Trophy, FileText, AlertCircle } from 'lucide-react';
...
const EditEventModal = ({ isOpen, onClose, onSuccess, event }) => {
  ...
  useEffect(() => {
    if (event && isOpen) {
      setFormData({
        ...
        registered_teams: event.registered_teams || 0,
        status: event.status || 'open',
        technical_meeting_date: event.technical_meeting_date ? event.technical_meeting_date.slice(0, 16) : '',
        match_date: event.match_date ? event.match_date.split('T')[0] : '',
        venue: event.venue || '',
  ...
  const validateForm = () => {
    ...
    // Validate registered_teams doesn't exceed max_teams
    const registered = parseInt(formData.registered_teams) || 0;
    const maxTeams = parseInt(formData.max_teams) || 0;
    if (registered > maxTeams) {
      newErrors.registered_teams = 'Registered teams tidak boleh lebih dari max teams';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const eventData = {
        ...
        registered_teams: parseInt(formData.registered_teams) || 0
      };

      await eventService.updateEvent(event.id, eventData);
      toast.success('Event berhasil diupdate!', { duration: 3000 });

      onSuccess();
      onClose();
    } catch (error) {
      console.error('Error updating event:', error);
      const errorMsg = error.response?.data?.message || 'Gagal mengupdate event';
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-700 sticky top-0 bg-gray-800 z-10">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">ID: {event.id}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  className={`w-full px-4 py-2 bg-gray-900/50 border ${
                    errors.registered_teams ? 'border-red-500' : 'border-gray-600'
                  } rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition-colors`}
                />
                {errors.registered_teams && (
                  <p className="mt-1 text-sm text-red-400">{errors.registered_teams}</p>
                )}
              </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-6 border-t border-gray-700">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="px-6 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-500 hover:to-orange-500 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-yellow-500/50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
            >
              {isSubmitting ? (
                <>
